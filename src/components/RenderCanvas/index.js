...
class RenderCanvas extends Component {
  constructor(props) {
    ...
    if (props.mapId) {
      axios.get(`${process.env.REACT_APP_API_URL}/api/${region}/${version}/map/${props.mapId}`)
  ...
  componentDidUpdate(prevProps) {
    ...
    if (prevProps.mapId != this.props.mapId && this.props.mapId) {
      axios.get(`${process.env.REACT_APP_API_URL}/api/${region}/${version}/map/${this.props.mapId}`)
        .then(response => {
          let stateChanges = {
      ...
      <div className={'canvas-bg' + (this.state.dragging ? ' dragging' : '')} style={{ backgroundPositionX: `${this.state.x}px`, backgroundPositionY: `${this.state.y}px` }}>
        <DraggableCore
          ...
          <div className={'canvas-characters' + (this.state.dragging ? ' dragging' : '')} onClick={this.clickCanvas.bind(this)} style={{ backgroundColor }}>
            ...
            {
              mapId ? <img className='map' src={`${process.env.REACT_APP_API_URL}/api/${region}/${version}/map/${mapId}/render`} draggable={false} onClick={this.clickCanvas.bind(this)} onError={this.mapLoadingError} /> : ''
            }
            {
